...
export class TimeComponent
  implements MatFormFieldControl<Time | string>, OnDestroy
{
  static nextId = 0;
  @HostBinding() id = `time-input-${TimeComponent.nextId++}`;
  stateChanges = new Subject<void>();
  @ViewChild('hoursInput', { static: true })
  _focusRef: ElementRef<HTMLInputElement>;
...
function parseTime(val: string): [number, number, 'AM' | 'PM'] {
  const values = val.split(/:|\s/),
    timeLimit = values[2] ? 12 : 24;
  if (
    values.length === 2 ||
    (values.length === 3 && ['AM', 'PM'].indexOf(values[2]) !== -1)
  ) {
