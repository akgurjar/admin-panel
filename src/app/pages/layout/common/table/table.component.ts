...
export class TableComponent implements OnInit {
  ...
  get displayedColumns(): string[] {
    if (this.tableSource) {
      ...
      const indexedColumns =
        this.tableSource.options && this.tableSource.options.index
          ? ['index', ...columns]
          : columns;
      return this.tableSource.options && this.tableSource.options.selection
        ? ['selection', ...indexedColumns]
        : indexedColumns;
    }
    return [];
  }
  ...
  get headerTemplate(): TemplateRef<any> {
    if (this.templates) {
      const header = this.templates.find(
        (template: ForDirective) => template.name === 'header'
      );
      if (header) {
        return header.ref;
      }
  ...
  get isSearchApplied(): boolean {
    return this.optionEvent.data.searchText?.length > 0;
  }
  ...
  get placeholder() {
    const search =
      this.tableSource &&
      this.tableSource.options &&
      this.tableSource.options.search;
  ...
  get hasSorting(): boolean {
    return (
      this.tableSource &&
      this.tableSource.options &&
      this.tableSource.options.sorting
    );
  }
  ...
  onFilterHandler() {
    ...
    if (
      this.tableSource &&
      this.tableSource.options &&
      this.tableSource.options.filterComponent
    ) {
      const subscription = this._dialog
        .open(this.tableSource.options.filterComponent, {
          disableClose: true,
          position: {
            right: '20px',
          },
  ...
  onSearchHandler(event: KeyboardEvent) {
    ...
    if (key === 'Enter') {
      ...
      if (
        value.length >= 3 ||
        (value.length === 0 && this.optionEvent.data.searchText)
      ) {
